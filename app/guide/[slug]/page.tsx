  ...
  return (
    <div className="app home">
      ...
      <div style={{ padding: '18px 18px 4px' }}>
        ...
        <p style={{ fontSize: 15, lineHeight: 1.8, color: '#3a3630', fontWeight: 500, margin: '0 0 18px' }}>{g.lead}</p>
        {g.sections.map((s, i) => (
        ...
        <Link href={g.ctaHref} style={cta}>{g.ctaText}<span style={{ display: 'block', fontSize: 13, fontWeight: 600, marginTop: 3, opacity: 0.9 }}>생년월일만 · 30초 무료로 시작</span></Link>
        {faqs.length > 0 && (<>
          <div style={{ fontFamily: 'var(--serif)', fontWeight: 800, fontSize: 15, color: 'var(--navy)', margin: '24px 0 10px' }}>자주 묻는 질문</div>
          {faqs.map((f, i) => (
            <div key={i} style={card}>
              <div style={{ fontFamily: 'var(--serif)', fontWeight: 800, fontSize: 15, color: 'var(--navy)', marginBottom: 6, lineHeight: 1.5 }}>Q. {f.q}</div>
              <p style={{ fontSize: 15, lineHeight: 1.8, color: '#33383f', margin: 0, fontWeight: 500 }}>{f.a}</p>
            </div>
          ))}
        </>)}
        {g.related && g.related.length > 0 && (<>
          <div style={{ fontFamily: 'var(--serif)', fontWeight: 800, fontSize: 15, color: 'var(--navy)', margin: '20px 0 10px' }}>함께 보면 좋은 글</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 24 }}>
            {g.related.map((r, i) => (<Link key={i} href={r.href} style={chip}>{r.label}</Link>))}
          </div>
        </>)}
        <p style={{ fontSize: 13, color: '#6d7882', lineHeight: 1.65, marginBottom: 20 }}>※ 만세력·십성·오행 상성으로 산출한 명리 기반 참고·오락용 정보입니다. 실제 투찰금액 산정의 근거로 사용할 수 없습니다.</p>
      </div>
    </div>
  );
}
