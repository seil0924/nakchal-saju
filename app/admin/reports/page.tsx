      ...
      <div className="abody">
        ...
        <div className="acard">
          <table><tbody>
            ...
            {reps.map((r, i) => (
              <tr key={i}><td style={{ fontFamily: 'monospace', fontSize: 12, color: '#a39c8e' }}>{String(r.id).slice(0, 8)}</td><td className="who">{r.name}</td><td>{r.corp}</td><td>{r.dir}</td><td><span className={'pill ' + (r.unlocked ? 'unlock' : 'lock')}>{r.unlocked ? '유료' : '무료'}</span></td><td style={{ color: '#a39c8e' }}>{r.at}</td><td><a href={`/report/${r.id}`} style={{ color: 'var(--gold)', fontWeight: 700, textDecoration: 'none', fontSize: 12 }}>열람 →</a></td></tr>
            ))}
