...
export default async function AdminDash() {
  ...
  return (
    <>
      ...
      <div className="abody">
        <div className="kpis">
          <div className="kpi"><div className="k"><span className="dot" style={{ background: '#2f4a7a' }} />총 회원</div><div className="v">{s.members.toLocaleString('ko-KR')}</div><div className="dl up">▲ 오늘 +{s.todaySignup}</div></div>
          <div className="kpi"><div className="k"><span className="dot" style={{ background: 'var(--gold)' }} />유료 전환</div><div className="v">{s.paid.toLocaleString('ko-KR')}<small> 명</small></div><div className="dl mut">전환율 {s.convRate}%</div></div>
          ...
          <div className="kpi"><div className="k"><span className="dot" style={{ background: '#2f6b57' }} />유료 회원</div><div className="v">{s.subs.toLocaleString('ko-KR')}<small> 명</small></div><div className="dl mut">1회 이상 결제</div></div>
          <div className="kpi"><div className="k"><span className="dot" style={{ background: '#7a5cae' }} />오늘 발급 리포트</div><div className="v">{s.todayReports.toLocaleString('ko-KR')}<small> 건</small></div><div className="dl mut">오늘 생성 기준</div></div>
