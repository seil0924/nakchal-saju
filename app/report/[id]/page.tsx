...
type Result = { reportId: string; title: string; unlocked: boolean; level?: number; wonguk?: Pillar[]; hero?: any; gauge?: any; sections: Section[] };
      ...
      <div className="wrap">
        ...
        {res && (
          <div className="rcols">
            ...
            {level < 2 && <div className="cta" onClick={() => { setErr(''); setSku('full'); setModal(true); }}>{level === 0 ? '잠긴 리포트 전체 열기' : '전체 리포트로 업그레이드'}<small>{level === 0 ? `잠긴 심층 해석 · 궁합 · 이달 택일 · 정밀 사정률까지 · ${won(PRICE_FULL)}` : `남은 심층 섹션까지 모두 · ${won(PRICE_FULL)}`}</small></div>}
