...
// 개인 심층값(원국·사정률·궁합)은 주지 않고 '유형 + 닮은 CEO'만 반환해 갈증을 만든다.
import { NextResponse } from 'next/server';
import { chartFromBirth, pil } from '@/lib/engine';
import { matchTycoon, TYPE_NAME, TYPE_DESC, TYPE_WAY } from '@/lib/tycoon';
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({
      type: TYPE_NAME[me], typeDesc: TYPE_DESC[me], way: TYPE_WAY[me],
      me,
