...
// 명식(원국) — 공통 코어 + 오행 분포(dist)
export function compute(y:number,m:number,d:number,hourFloat:number|null,yaja=false):Chart{
  const p=corePillars(y,m,d,hourFloat,yaja);
  const dist=[0,0,0,0,0];
  dist[GAN_EL[p.yGan]]++;dist[ZHI_EL[p.yZhi]]++;dist[GAN_EL[p.mGan]]++;dist[ZHI_EL[p.mZhi]]++;dist[GAN_EL[p.dGan]]++;dist[ZHI_EL[p.dZhi]]++;
  if(p.hGan!==null){dist[GAN_EL[p.hGan]]++;dist[ZHI_EL[p.hZhi!]]++;}
  return {...p, dist};
}
// 생일 → 명식 (서머타임·진태양시·야자시·음력 보정은 resolveBirth가 처리)
export function chartFromBirth(dateISO:string,timeHHMM:string|null,cal:'solar'|'lunar'='solar',isLeap=false):Chart{
  ...
  return compute(r.y,r.m,r.d,r.hf,r.yaja);
}
...
export function sinsal(c:Chart):Sinsal[]{
  ...
  if(['6_4','6_10','8_4','8_10','4_10'].includes(dj))
    out.push({key:'괴강',name:'괴강살',hanja:'魁罡殺',
      ...
      body:'일주에 <b>괴강</b>이 앉았습니다. 만인을 이끄는 극단의 리더 부호라, 어중간을 못 견디고 끝을 봐야 직성이 풀리셨을 겁니다. 평사원으로는 답답해 스스로 판을 벌이셨을 자리 — 다만 그 강함이 사람을 눌러 홀로 서기도 쉬우니, 힘을 거둘 때를 아는 것이 그릇의 완성입니다.'});
  ...
  if(['0_4','1_7','2_10','3_1','4_4','8_10','9_1'].includes(dj))
    out.push({key:'백호',name:'백호살',hanja:'白虎殺',
      ...
      body:'일주에 <b>백호</b>가 실렸습니다. 판이 커질수록 살아나는 기질이라, 남들이 몸 사릴 때 대표님은 오히려 크게 지르셨을 겁니다. 그 배포가 오늘을 만들었으나, 한 번의 큰 건에 전부를 거는 습관은 화가 되니 하한선을 정해두고 움직이십시오.'});
  // 양인살 — 양간 일간이 제왕지를 볼 때
  ...
  if(yangin[c.dGan]!==undefined && c.dZhi===yangin[c.dGan])
    out.push({key:'양인',name:'양인살',hanja:'羊刃殺',
      ...
      body:'일지에 <b>양인</b>이 들었습니다. 한번 방향이 서면 좌우 안 보고 끝까지 미는 강력한 추진력 — 큰 건을 뚫어내는 대표님의 진짜 힘입니다. 다만 그 칼끝이 안으로 향하면 무리한 저가·과속으로 스스로를 베니, 결단의 순간마다 물러설 선을 함께 정하십시오.'});
  // 천을귀인 — 일간별 귀인 지지
  ...
  if(has(zhis,gwiin[c.dGan]||[]))
    out.push({key:'천을',name:'천을귀인',hanja:'天乙貴人',
      ...
      body:'명식에 <b>천을귀인</b>이 있습니다. 사주 중 으뜸으로 치는 길신 — 벼랑 끝이라 생각한 자리에서 뜻밖의 사람·발주처가 손을 내밀어준 경험이 한두 번이 아니셨을 겁니다. 이 복은 사람에게 정을 들일수록 커지니, 관계를 넓게 오래 보십시오.'});
  // 현침살 — 甲午·甲申·辛卯·辛未 일주
  if(['0_6','0_8','7_3','7_7'].includes(dj))
    out.push({key:'현침',name:'현침살',hanja:'懸針殺',
      head:'입만 열면 정곡을 찌르는, 바늘 같은 예리함',
      body:'일주에 <b>현침</b>이 섰습니다. 남이 못 보는 허점을 단번에 짚어내는 날카로운 통찰 — 협상 테이블에서 대표님의 무기입니다. 다만 그 말이 바늘이라 옳은 말도 상처를 남기니, 정확함 뒤에 한마디 온기를 더하면 사람이 남습니다.'});
  // 역마살 — 지지 寅申巳亥
  if(has(zhis,[2,8,5,11]))
    out.push({key:'역마',name:'역마살',hanja:'驛馬殺',
      head:'한자리에 못 앉는, 넓힐수록 사는 확장의 기운',
      body:'명식에 <b>역마</b>가 붙었습니다. 가만히 지키는 것보다 현장을 돌고 판을 넓힐 때 운이 트이는 기질 — 지역·업역을 넓히는 확장이 대표님께 유리합니다. 다만 벌인 판이 많아 관리에서 새기 쉬우니, 뻗을 때일수록 안살림 챙길 사람을 곁에 두십시오.'});
  // 화개살 — 일지 辰戌丑未
  if([4,10,1,7].includes(c.dZhi) && out.length<2)
    out.push({key:'화개',name:'화개살',hanja:'華蓋殺',
      head:'무리 속에서도 끝내 혼자인, 고독한 장인의 별',
      body:'일지에 <b>화개</b>가 앉았습니다. 사람은 많이 겪되 속을 다 내주진 않는, 한 분야를 깊이 파는 전문가의 부호입니다. 외로우셨을 겁니다 — 허나 그 고독이 남들 못 따라오는 전문성을 벼렸으니, 이제는 그 깊이를 사람 키우는 데 나눠 쓰실 때입니다.'});
