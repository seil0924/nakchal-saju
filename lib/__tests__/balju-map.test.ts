...
describe('sipsungOf', () => {
  it('같은 오행이면 음양에 따라 비견/겁재', () => {
    expect(sipsungOf(0, 0)).toBe('비견');   // 갑↔갑
    expect(sipsungOf(0, 1)).toBe('겁재');   // 갑↔을
  });

  it('내가 생하는 오행이면 식신/상관', () => {
    expect(sipsungOf(0, 2)).toBe('식신');   // 목→화, 양양
    expect(sipsungOf(0, 3)).toBe('상관');   // 목→화, 양음
  });

  it('내가 극하는 오행이면 편재/정재', () => {
    expect(sipsungOf(0, 4)).toBe('편재');   // 목→토, 양양
    expect(sipsungOf(0, 5)).toBe('정재');   // 목→토, 양음
  });

  it('나를 극하는 오행이면 편관/정관', () => {
    expect(sipsungOf(0, 6)).toBe('편관');   // 금→목, 양양
    expect(sipsungOf(0, 7)).toBe('정관');   // 금→목, 양음
  });

  it('나를 생하는 오행이면 편인/정인', () => {
    expect(sipsungOf(0, 8)).toBe('편인');   // 수→목, 양양
    expect(sipsungOf(0, 9)).toBe('정인');   // 수→목, 양음
  });

  it('10천간 전 조합에서 십성 10종이 빠짐없이 나온다', () => {
    const seen = new Set<string>();
    for (let me = 0; me < 10; me++) {
      for (let td = 0; td < 10; td++) seen.add(sipsungOf(me, td));
    }
...
describe('jiModifier', () => {
  ...
  it('합이 12로 떨어지면 육합', () => {
    expect(jiModifier(1, 0).name).toBe('육합');   // 축+자
    expect(jiModifier(2, 11).name).toBe('육합');  // 인+해
  });
  it('세 칸·아홉 칸이면 형', () => {
...
describe('fitOf', () => {
  it('설립연도로 년주 간지와 오행을 뽑는다', () => {
    ...
    expect(f.ganji).toBe('甲子');   // 1984 = 갑자년
    expect(f.oh).toBe(0);           // 갑 = 목
  });

  it('핵심 발주처는 같은 조건에서 점수가 더 높다', () => {
    const a = fitOf({ gan: 0, ji: 0 }, C('일반', '1990-01-01'));
    const b = fitOf({ gan: 0, ji: 0 }, C('핵심', '1990-01-01', true));
    expect(b.score).toBe(a.score + 3);
  });

  it('점수는 38~99 범위를 벗어나지 않는다', () => {
    for (let g = 0; g < 10; g++) {
      for (let j = 0; j < 12; j++) {
        for (const c of CLIENTS) {
          const f = fitOf({ gan: g, ji: j }, c);
          expect(f.score).toBeGreaterThanOrEqual(38);
...
describe('rankBalju', () => {
  ...
  it('같은 입력이면 순서가 항상 같다 (새로고침마다 순위가 바뀌면 안 된다)', () => {
    const a = rankBalju(me).map(x => x.name);
    const b = rankBalju(me).map(x => x.name);
    expect(a).toEqual(b);
  });
...
describe('splitForMap', () => {
  ...
  it('잠기는 쪽이 항상 더 높은 점수다 (가장 좋은 곳을 잠근다)', () => {
    ...
    const worstLocked = Math.min(...locked.map(x => x.score));
    const bestFree = Math.max(...free.map(x => x.score));
    expect(worstLocked).toBeGreaterThanOrEqual(bestFree);
  });
