...
export function loadPeople(): Person[] {
  ...
  try { const s = localStorage.getItem(scopedKey(KEY)); return s ? JSON.parse(s) : []; } catch { return []; }
}
...
export function savePerson(p: Partial<Person> & { kind: PersonKind; date: string }): Person {
  ...
  arr.unshift(rec);
  try { localStorage.setItem(scopedKey(KEY), JSON.stringify(arr.slice(0, 60))); } catch {}
  return rec;
}
export function removePerson(id: string) {
  const arr = loadPeople().filter(x => x.id !== id);
  try { localStorage.setItem(scopedKey(KEY), JSON.stringify(arr)); } catch {}
}
...
export function migrateLegacy() {
  if (typeof window === 'undefined') return;
  try {
    if (localStorage.getItem(scopedKey(KEY))) return; // 이미 통합됨
    const out: Person[] = [];
    const self = JSON.parse(localStorage.getItem(scopedKey('nakchal_self_v1')) || '[]');
    ...
    const legal = JSON.parse(localStorage.getItem(scopedKey('nakchal_legal_v1')) || '[]');
    for (const l of legal) if (l?.legal) out.push({ id: newId(), kind: 'legal', name: l.company || '', date: l.legal, cal: 'solar' });
    const tg = JSON.parse(localStorage.getItem(scopedKey('nakchal_saved_targets_v1')) || '[]');
    ...
    if (out.length) localStorage.setItem(scopedKey(KEY), JSON.stringify(out.slice(0, 60)));
  } catch {}
}
